require 'pp'
require_relative "../../utils/sentence_expand.rb"

SExpand = BeerBot::Modules::Utils::SentenceExpand
...
describe "sentence sentence expansion", :param_expand => true do
  ...
  it "should generate sentences with a valid spec" do
    100.times do
      ...
      SExpand.expand(sentence,params1)
    end
  end
  ...
  it "can raise an error if key lookups don't exist" do
    sentence = ":zzz :youmust :dothis :toget :this ::from"
    expect{
      SExpand.expand(sentence,params1,true)
    }.to raise_error
  end
